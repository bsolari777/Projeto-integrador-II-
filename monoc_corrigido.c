    ...
    while(*pc >= 0 && *pc <= 255 ){ 
        ...
        //   1000 (8)  = BEQ
			...
			printf("...OVERFLOW....\n");
                ...
                int ula_result = ULA(1, reg[rs2], reg[rt2],&f);
                if(f.zero == 1){ // se a flag de zero for == 1 ( verdadeira ) realiza o branch
                    proximo_pc = *pc + imediato2 +1; 
                }
       ...
       // Para Jump, o endereço é sempre positivo (0-255)  
          ...
          if(addr_s[i] == '1'){
            ...
            proximo_pc = addr; // -1 porque o (*pc)++ vai incrementar depois
...
void salvar_asm(struct memoria_instrucao mem[], int qtd_instr){
    ...
    for(int i=0; i<qtd_instr; i++){
        ...
        // -------- TIPO J --------
            ...
            int addr = 0;
            for(int j = 0; j < 12; j++){
                if(addr_s[j] == '1'){			//binario para decimal	
					addr = addr * 2 + 1;
				}else{						
				addr = addr * 2 + 0;
				}
